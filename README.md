# Безопасный SQL Runner (Ubuntu / Debian)

Простой CLI-скрипт на Python, который безопасно выполняет **только SELECT**-запросы к PostgreSQL:
блокирует любые изменяющие данные операторы и автоматически добавляет `LIMIT 5`,
если лимит не указан явно.

Без веб-сервера. Без LLM. Только Python + PostgreSQL.
